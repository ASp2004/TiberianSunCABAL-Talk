...
class TibSunCabal:
    def __init__(self):
        ...
        cabal_online_rec = np.rec.array([
            ('', 0, 2),('K', 44, 2),('AH', 161, 1), ('B', 262, 2),('AE', 594, 2),('L', 840, 0),('', 882, 2),#CABAL
            ('AA', 1119, 2), ('N', 1163, 2),('L', 1430, 2),('AY', 1607, 2),('', 1876, 2), #ONLI-
            ('', 1994, 1), 
        ], names=('phoneme', 'timing', 'priority'))
        
        self.best_cabal_clip = cabal_online[:840]
    ...
    def load_text(self, text: str = None, progress_callback: Callable[[int, int, int], None] = None) \
        ...
        while len(phonemes) > 0:
            ...
            if p == ' ':
                ...
                output = self.append_audio_segment(output, self.best_cabal_clip, -1, audio_out_millis,
                                                   audio_source_millis)
    ...
    def append_audio_segment(audio_out: AudioSegment, audio_segment: AudioSegment, source_millis_start: int,
        ...
        audio_out_millis.append(audio_length)
